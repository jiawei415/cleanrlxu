...
class Net(nn.Module):
    def __init__(self, units=128, label_num=18, dueling=False):
        ...
        if self.dueling:
            ...
            self.dqn_out = nn.Linear(self.units * 2, self.label_num, True)
            self.ppo_out = nn.Linear(self.units * 2, self.label_num + 1, True)
    ...
    def forward(self, entities, localmap, globalmap):
        ...
        dqn_out = self.dqn_out(preout)
        ppo_out = self.ppo_out(preout)

        return dqn_out, ppo_out


@registry.register("torch_mix_asym_model")
class _TorchAsymModelImplV5(_TorchModelImpl):
    def __init__(
        self,
    ...
    def forward(self, input_dict):
        ...
        for i in range(agent_num):
            ...
            dqn_out, ppo_out = self.policy_net(entities, localmap, globalmap)
            p["qs"] = dqn_out
            p["logits"] = ppo_out[:, 0 : self._label_num]  # noqa E203
            p["value"] = ppo_out[:, self._label_num]
            ...
            if "train" in input_dict and input_dict["train"]:
                if not self.n_step:
                    ...
                    with torch.no_grad():
                        p["next_target_qs"], _ = self.target_net(
                            next_entities, next_localmap, next_globalmap
                        )
                        if self.double:
                            p["next_qs"], _ = self.policy_net(
                                next_entities, next_localmap, next_globalmap
                            )
                else:
                    ...
                    with torch.no_grad():
                        p["n_step_target_qs"], _ = self.target_net(
                            n_step_entities, n_step_localmap, n_step_globalmap
                        )
                        if self.double:
                            p["n_step_qs"], _ = self.policy_net(
                                n_step_entities, n_step_localmap, n_step_globalmap
                            )
