...
if __name__ == "__main__":
    ...
    # Transfer config
    parser.add_argument('--source', type=str, default="Pong",
                        help='the name of sourece game')
    parser.add_argument('--source-path', type=str, default='20210425121300',
...
source_path = f"results/{source_game}_mppo_{args.source_path}/checkpoints/best_model.pt"

# TRY NOT TO MODIFY: setup the environment
now_time = time.strftime("%Y%m%d%H%M%S", time.localtime())
game_name = args.gym_id + "NoFrameskip-v4"
print(f"play game: {game_name}")
experiment_name = f"{game_name}_policy_{now_time}"
...
teacher_actions = torch.zeros((args.num_steps, args.num_envs) + source_envs.action_space.shape).to(device)
...
for update in range(1, num_updates+1):
    ...
    for step in range(0, args.num_steps):
        ...
        with torch.no_grad():
            ...
            teacher_action, _, _, _ = teacher.get_action(obs[step])
        actions[step] = action
    ...
    b_teacher_actions = teacher_actions.reshape((-1,)+source_envs.action_space.shape)
    ...
    for i_epoch_pi in range(args.update_epochs):
        ...
        for start in range(0, args.batch_size, args.minibatch_size):
            ...
            _, teacherlogproba, _, hidden_teacher = teacher.get_action(b_obs[minibatch_ind], b_teacher_actions.long()[minibatch_ind])
            # Policy loss
            entropy_loss = entropy.mean()
            pg_loss = torch.exp(teacherlogproba) * studentlogproba
