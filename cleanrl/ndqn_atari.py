...
class QNetwork(nn.Module):
    def __init__(self, env, frames=4):
        ...
        self.advs = nn.Linear(512, env.action_space.n)
...
for global_step in range(start_step+1, args.total_timesteps):
    ...
    tmp_q.append((obs, action, reward, next_obs, done))
    if len(tmp_q) == n_step:
        # n_step_reward = tmp_q[0][2] + args.gamma * tmp_q[1][2] + (args.gamma ** 2) * tmp_q[2][2]
        n_step_reward = sum([trans[2] * (args.gamma**i) for i, trans in enumerate(tmp_q)])
        n_step_obs = tmp_q[-1][0]
        rb.put((tmp_q[0][0], tmp_q[0][1], n_step_reward, n_step_obs, tmp_q[0][4]))
    ...
    if global_step > args.learning_starts and global_step % args.train_frequency == 0:
        ...
        with torch.no_grad():
            next_qs = q_network.forward(s_next_obses, device)
            next_target_qs = target_network.forward(s_next_obses, device)
            next_target_max_q = next_target_qs.gather(1, next_qs.max(1)[1].unsqueeze(1)).squeeze()
            target_q = torch.Tensor(s_rewards).to(device) + args.gamma * next_target_max_q * (1 - torch.Tensor(s_dones).to(device))
        q = q_network.forward(s_obs, device).gather(1, torch.LongTensor(s_actions).view(-1,1).to(device)).squeeze()
    ...
    if done:
        while tmp_q:
            obs, action, reward, next_obs, done = tmp_q.popleft()
            n_step_reward = sum([trans[2] * (args.gamma**i) for i, trans in enumerate(tmp_q)]) if tmp_q else reward
            n_step_obs = tmp_q[-1][0] if tmp_q else next_obs
            rb.put((obs, action, n_step_reward, n_step_obs, done))
        obs = env.reset()
