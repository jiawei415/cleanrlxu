...
ckpt = args.ckpt
if ckpt:
    ckpt_load_path = f"results/{game_name}_c51_{ckpt}/checkpoints/best_ckpt.pkl"
...
for i in range(100):
    ...
    while not done:
        env.render()
        action, pmf = q_network.get_action(env, device, obs.reshape((1,)+obs.shape))
        ...
        if done:
            obs = env.reset()
            print(f"i: {i}, step: {step}, reward: {episode_reward}")
env.close()
